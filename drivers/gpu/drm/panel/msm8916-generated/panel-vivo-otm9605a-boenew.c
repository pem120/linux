...
struct pd1505l_otm9605a_540p_boenew {
...
static const struct regulator_bulk_data pd1505l_otm9605a_540p_boenew_supplies[] = {
	{ .supply = "vdd3" },
	{ .supply = "vci" },
};

static inline
struct pd1505l_otm9605a_540p_boenew *to_pd1505l_otm9605a_540p_boenew(struct drm_panel *panel)
{
	return container_of_const(panel, struct pd1505l_otm9605a_540p_boenew, panel);
}

static void pd1505l_otm9605a_540p_boenew_reset(struct pd1505l_otm9605a_540p_boenew *ctx)
{
...
static int pd1505l_otm9605a_540p_boenew_on(struct pd1505l_otm9605a_540p_boenew *ctx)
{
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xc5, 0x21);
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xf5,
					 0x15, 0x00, 0x15, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xd8, 0x67, 0x67);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xe1,
					 0x00, 0x07, 0x0f, 0x0e, 0x06, 0x10,
					 0x0b, 0x09, 0x04, 0x07, 0x0c, 0x07,
					 0x0e, 0x11, 0x0a, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xe2,
					 0x00, 0x08, 0x0e, 0x0e, 0x07, 0x0f,
					 0x0b, 0x0a, 0x03, 0x07, 0x0c, 0x08,
					 0x0f, 0x10, 0x0a, 0x00);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0x80);
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0xa0);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xce,
					 0x18, 0x05, 0x83, 0xc0, 0x87, 0x1d,
					 0x0d, 0x18, 0x04, 0x83, 0xc1, 0x86,
					 0x1d, 0x0d);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0xb0);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xce,
					 0x18, 0x03, 0x83, 0xc2, 0x87, 0x1d,
					 0x0d, 0x18, 0x02, 0x83, 0xc3, 0x86,
					 0x1d, 0x0d);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0xc0);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xce,
	...
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0xed,
					 0x40, 0x34, 0x44, 0x44, 0x34, 0x44,
					 0x44, 0x43, 0x44, 0x34, 0x44, 0x44,
					 0x34, 0x44, 0x44, 0x43, 0x44, 0x34,
					 0x44, 0x44, 0x34, 0x44, 0x44, 0x43,
					 0x44, 0x34, 0x44, 0x44, 0x34, 0x44,
					 0x44, 0x43, 0x04);
	mipi_dsi_generic_write_seq_multi(&dsi_ctx, 0x00, 0x00);
...
static int pd1505l_otm9605a_540p_boenew_off(struct pd1505l_otm9605a_540p_boenew *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int pd1505l_otm9605a_540p_boenew_prepare(struct drm_panel *panel)
{
	struct pd1505l_otm9605a_540p_boenew *ctx = to_pd1505l_otm9605a_540p_boenew(panel);
	...
	pd1505l_otm9605a_540p_boenew_reset(ctx);

	ret = pd1505l_otm9605a_540p_boenew_on(ctx);
	if (ret < 0) {
...
static int pd1505l_otm9605a_540p_boenew_unprepare(struct drm_panel *panel)
{
	struct pd1505l_otm9605a_540p_boenew *ctx = to_pd1505l_otm9605a_540p_boenew(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = pd1505l_otm9605a_540p_boenew_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode pd1505l_otm9605a_540p_boenew_mode = {
	.clock = (540 + 35 + 4 + 100) * (960 + 15 + 4 + 16) * 60 / 1000,
	.hdisplay = 540,
	.hsync_start = 540 + 35,
	.hsync_end = 540 + 35 + 4,
	.htotal = 540 + 35 + 4 + 100,
...
static int pd1505l_otm9605a_540p_boenew_get_modes(struct drm_panel *panel,
						  struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &pd1505l_otm9605a_540p_boenew_mode);
}

static const struct drm_panel_funcs pd1505l_otm9605a_540p_boenew_panel_funcs = {
	.prepare = pd1505l_otm9605a_540p_boenew_prepare,
	.unprepare = pd1505l_otm9605a_540p_boenew_unprepare,
	.get_modes = pd1505l_otm9605a_540p_boenew_get_modes,
};

static int pd1505l_otm9605a_540p_boenew_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct pd1505l_otm9605a_540p_boenew *ctx;
	int ret;

	ctx = devm_drm_panel_alloc(dev, struct pd1505l_otm9605a_540p_boenew, panel,
				   &pd1505l_otm9605a_540p_boenew_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(pd1505l_otm9605a_540p_boenew_supplies),
					    pd1505l_otm9605a_540p_boenew_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
...
static void pd1505l_otm9605a_540p_boenew_remove(struct mipi_dsi_device *dsi)
{
	struct pd1505l_otm9605a_540p_boenew *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id pd1505l_otm9605a_540p_boenew_of_match[] = {
	{ .compatible = "vivo,otm9605a-boenew" }, // FIXME
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, pd1505l_otm9605a_540p_boenew_of_match);

static struct mipi_dsi_driver pd1505l_otm9605a_540p_boenew_driver = {
	.probe = pd1505l_otm9605a_540p_boenew_probe,
	.remove = pd1505l_otm9605a_540p_boenew_remove,
	.driver = {
		.name = "panel-vivo-otm9605a-boenew",
		.of_match_table = pd1505l_otm9605a_540p_boenew_of_match,
	},
};
module_mipi_dsi_driver(pd1505l_otm9605a_540p_boenew_driver);

MODULE_AUTHOR("linux-mdss-dsi-panel-driver-generator <fix@me>"); // FIXME
MODULE_DESCRIPTION("DRM driver for pd1505l otm9605a 540p boenew video mode dsi panel");
MODULE_LICENSE("GPL");
